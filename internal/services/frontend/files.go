...
func serveFileStatic(path string) http.Handler {
    fp := filepath.Join(server.rootFilePath, path)

    return http.FileServer(http.Dir(fp))
}

func serveFileTemplate(w http.ResponseWriter, r *http.Request) {
    lp := filepath.Join(server.rootFilePath, "templates", "layout.html")
    fp := filepath.Join(server.rootFilePath, "templates", filepath.Clean(r.URL.Path))
